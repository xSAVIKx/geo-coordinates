<script lang="ts">
  ...
  onMount(() => {
    mapState.applyScene({ views: ['globe', 'flat'], point: HOME, pointEditable: true, layers: { graticuleStep: 'auto', specialLines: true, tropics: true, daylight: true }, sun: { utcMinutes: 720, dayOfYear: 80 }, overlays: [{ kind: 'noon-meridian' }], labControls: ['sun-time', 'sun-date', 'now', 'clocks'] });
    mapState.setSunNow();
  });
